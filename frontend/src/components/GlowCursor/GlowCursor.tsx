...
export const GlowCursor: React.FC<GlowCursorProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      renderer = new Renderer({
        canvas,
        alpha: true,
        dpr: Math.min(typeof window !== 'undefined' ? window.devicePixelRatio || 1 : 1, initialConfig.maxDevicePixelRatio ?? 1.5)
      });
      gl = renderer.gl;
      if (!gl) return;
      gl.clearColor(0, 0, 0, 0);
    } catch {
      // Gracefully handle environments without WebGL support (e.g. headless jsdom)
      return;
    }
    ...
    let fade = 0;
    let lastInputTime = typeof performance !== 'undefined' ? performance.now() : Date.now();
    ...
    const updatePointer = (event: PointerEvent) => {
      ...
      target.y = y;
      pointerInside = true;
      lastInputTime = typeof performance !== 'undefined' ? performance.now() : Date.now();
    };
    ...
    const render = (now: number) => {
      if (destroyed || !renderer || !program || !mesh) return;
      const config = propsRef.current;
      const delta = Math.min((now - lastFrameTime) / 16.667, 3);
      lastFrameTime = now;

      if (initialized) {
        const follow = config.followSpeed ?? 0.16;
  ...
  return (
    <div
      ...
      style={style}
      data-testid="glow-cursor-container"
      {...rest}
    >
      <canvas
        ref={canvasRef}
      ...
      />
      {children && (
        <div className="glow-cursor__content" data-testid="glow-cursor-content">
          {children}
        </div>
      )}
    </div>
