...
export const Lightfall: React.FC<LightfallProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      const resize = () => {
        ...
        uniforms.iResolution.value = [gl.drawingBufferWidth || width, gl.drawingBufferHeight || height, 1];
      };
      ...
      const loop = (t: number) => {
        ...
        if (!paused && programRef.current && meshRef.current && renderer) {
          try {
            ...
            console.error('Lightfall render error:', err);
          }
        }
      };
      ...
      console.warn('WebGL/OGL not supported in current environment:', e);
      return;
    }
  }, [
    dpr,
  ...
  return (
    <div
      ...
      data-testid="lightfall-container"
    >
      {children}
    </div>
  );
